...
<div class="container">
    ...
    <div class="bg-white p-4 rounded shadow-sm mb-4">
        <form action="{{ isset($province) ? route('provinces.update', $province) : route('provinces.store') }}" method="POST" id="provinceForm">
            @csrf
            @if(isset($province))
                @method('PUT')
            @endif
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const formInputs = document.querySelectorAll('#provinceForm input[type="text"], #provinceForm input[type="number"], #provinceForm select');
